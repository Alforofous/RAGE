...
namespace RAGE {
    ...
    concept GpuBuffer = requires(const T buf) {
        ...
        {
            buf.usage()
        }->std::convertible_to<BufferUsage>;
        {
            buf.mappedData()
    ...
    concept GpuImage = requires(const T img, ImageViewCreateInfo viewInfo) {
        {
            img.format()
        }->std::convertible_to<ImageFormat>;
        {
            img.extent()
        }->std::convertible_to<Extent3D>;
        {
            img.mipLevels()
    ...
    concept GpuAllocator = requires(T alloc, BufferCreateInfo bufInfo, ImageCreateInfo imgInfo) {
        ...
        {
            alloc.createBuffer(bufInfo)
        }->std::same_as<typename T::Buffer>;
        {
            alloc.createImage(imgInfo)
        }->std::same_as<typename T::Image>;
    } && requires {
