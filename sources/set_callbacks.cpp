...
static void mouse_button_callback(GLFWwindow *window, int button, int action, int modifiers)
{
	...
	rage->nanogui_screen->mouseButtonCallbackEvent(button, action, modifiers);
}
...
//	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
	...
	rage->nanogui_screen->cursorPosCallbackEvent(x, y);
}
...
static void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
	...
	rage->nanogui_screen->keyCallbackEvent(key, scancode, action, mods);
}
