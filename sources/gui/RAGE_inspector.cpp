...
void RAGE_inspector::draw(RAGE *rage)
{
	...
	for (int i = 0; i < selected_objects->size(); i++)
	{
		...
		draw_drag_float3(rage, "Rotation" + std::to_string(i), selected_object->rotation, 1.0f, &min_rotation, &max_rotation, true);
		draw_drag_float3(rage, "Scale" + std::to_string(i), selected_object->scale, 0.1f);
		ImGui::Separator();
	}
	ImGui::End();
}