...
void RAGE_inspector::draw(RAGE *rage)
{
	...
	ImGui::Text("Camera:");
	glm::vec3 camera_position = rage->camera.m_position;
	...
	for (int i = 0; i < selected_objects->size(); i++)
	{
		...
		draw_drag_float3(rage, "Position" + std::to_string(i), selected_object->m_position, 0.1f);
		glm::vec3 min_rotation = glm::vec3(-180.0f);
		glm::vec3 max_rotation = glm::vec3(180.0f);
		draw_drag_float3(rage, "Rotation" + std::to_string(i), selected_object->m_rotation, 1.0f, &min_rotation, &max_rotation, true);
		draw_drag_float3(rage, "Scale" + std::to_string(i), selected_object->m_scale, 0.1f);
		ImGui::Separator();
	}
	ImGui::End();
}