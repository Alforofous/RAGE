...
public:
    ...
    Vector3 toVector3() const { return { toFloat(this->vector.x), toFloat(this->vector.y), toFloat(this->vector.z) }; }
...
private:
    ...
    glm::ivec3 vector;
    //Extract elsewhere later
    static float toFloat(const int value) { return static_cast<float>(value); }
};